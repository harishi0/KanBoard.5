...
class App:
    ...
    def __init_menu__(self, background_color):
        self.background_color = background_color
        self.screen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)
        self.clock = pygame.time.Clock()
        pygame.font.init()

    def create_buttons(self, BLACK):
        # Define the buttons
        button_width = 100
        button_height = 30
        button_margin = 10
        ...
        start_x = (App.WIDTH - total_width) // 2
        tab_y = button_margin
        ...
        pygame.draw.rect(self.screen, BLACK, self.tab1)  # Draw the button with the BLACK color
        pygame.draw.rect(self.screen, BLACK, self.tab2)
        pygame.draw.rect(self.screen, BLACK, self.tab3)
        pygame.draw.rect(self.screen, BLACK, self.tab4)

        button_font = pygame.font.SysFont("Arial", 15)
        text1 = button_font.render("Kanban Board", True, BLACK)
        text2 = button_font.render("Whiteboard", True, BLACK)
        text3 = button_font.render("Calendar", True, BLACK)
        text4 = button_font.render("Timer", True, BLACK)
    ...
    def __init_whiteboard__(self, WIDTH, HEIGHT, background_color):
        pygame.init()
    ...
    def handle_mouse_button(self, pos, background_color):
    ...
    def draw(self, pos, BLACK, BLUE, background_color):
        if self.drawing:
    ...
    def run(self):
        ...
        App.WIDTH = screen_info.current_w
        App.HEIGHT = screen_info.current_h
        self.screen = pygame.display.set_mode((App.WIDTH, App.HEIGHT), pygame.FULLSCREEN)
        running = True
        while running:
            for event in pygame.event.get():
    ...
    app = App(values()[-1])  # Pass the background_color from values()
